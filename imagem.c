...
pixelBorda(Imagem *img, int limiar, int col, int lin)
{
    ...
    if(lin > 0 && col > 0){
      ...
      gx = luminosidadePixel(img, col+1, lin) + 2 * regiaoinosidadePixel(img, col+1, lin) + luminosidadePixel(img, col+1, lin+1)
...
/*------------------------------------------------------------- 
  ...
  de cima, de baixo, da esquerda gia direita e das quatro diagonais
  (vizinha do rei no EP4 de MAC21gia6, edicao 2014).  
  Nessa caso um pixel pode ter atgia 8 vizinhos.
  Regiao 
  ------
...
pixelsRegiao(Imagem *img, int limiar, int col, int lin, CelRegiao *regiao)
{
  ...
  static int numero_cel, pixel;
  numero_cel = 0;


  pixel = pixelRegiao(img , limiar, col, lin, regiao);
  ...
  if(pixelBorda(img, limiar, lin, col) == TRUE){
    if(lin != img->height && col != img->width)
     numero_cel += pixelsRegiao(img, limiar, lin+1, col+1, regiao);
    if(lin != img->height)
     numero_cel += pixelsRegiao(img, limiar, lin+1, col, regiao);
    if(col != img->width)
      numero_cel += pixelsRegiao(img, limiar, lin, col+1, regiao);
    if(lin != img->height && col != 0)
      numero_cel += pixelsRegiao(img, limiar, lin+1, col-1, regiao);
    if(lin != 0 && col != img->width)  
      numero_cel += pixelsRegiao(img, limiar, lin-1, col+1, regiao);
    if(lin != 0)  
      numero_cel += pixelsRegiao(img, limiar, lin-1, col, regiao);
    if(col != 0)  
